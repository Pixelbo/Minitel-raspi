...
class Server(object):
    ...
    def request(self, command_string, preserve_encoding=False):
        ...
        if not preserve_encoding:
            #print(self.__unquote(self.__decode(response)))
    ...
    def request_with_results(self, command_string, preserve_encoding=False):
        ...
        try:
            #init
            quotedColon = urllib.parse.quote_plus(':')
            ...
            for result in results:
                ...
                if len(result) > 0:
                    ...
                    for subResult in subResults:
                        ...
                        if not preserve_encoding:
                            item[urllib.parse.unquote_plus(key)] = self.__unquote(value)
                        else:
                            item[key] = value
                    output.append(item)
    ...
    def search(self, term, mode='albums'):
        ...
        if mode == 'albums':
            return self.request_with_results(
                "albums 0 50 tags:%s search:%s" % ("la", term))
        elif mode == 'songs':
            return self.request_with_results(
                "songs 0 50 tags:%s search:%s" % ("lau", term))
        elif mode == 'artists':
            return self.request_with_results(
    ...
    def __quote(self, text):
        try:
            import urllib.parse
            return urllib.parse.quote_plus(text, encoding=self.charset)
        except ImportError:
            import urllib
